...
class TestUnitPropagationPerformance:
    ...
    def test_unit_propagation_performance(self):
        ...
        assert resolution_time < 1.0, (
            f"Resolution took {resolution_time:.2f}s, expected < 1.0s"
        )
    ...
    def test_unit_propagation_with_conflicts(self):
        ...
        assert resolution_time < 2.0, (
            f"Resolution took {resolution_time:.2f}s, expected < 2.0s"
        )
    ...
    def test_large_incompatibility_set(self):
            ...
            for j in range(min(3, len(packages) - i - 1)):
                if i + j + 1 < len(packages):
                    provider.add_dependency(
                        ...
                        Version("1.0.0"),
                        Dependency(
                            packages[i + j + 1], VersionRange(Version("1.0.0"), None)
                        ),
                    )
        ...
        assert resolution_time < 3.0, (
            f"Resolution took {resolution_time:.2f}s, expected < 3.0s"
        )
