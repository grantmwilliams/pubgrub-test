            ...
            if available_deps and random.random() < dependency_probability:
                ...
                for dep_package_name in dep_packages:
                    ...
                    if len(dep_versions) == 1:
                        ...
                        dep_range = VersionRange(dep_versions[0], dep_versions[0], True, True)
                    else:
                        # Random range
        ...
        if result.success and result.solution:
            ...
            for assigned_package_name, assigned_version in assignments.items():
                ...
                for dependency in dependencies:
                    ...
                    assert dep_package_name in assignments, f"Dependency {dep_package_name} not in solution"
                    dep_assigned_version = assignments[dep_package_name]
                    assert dep_range.contains(dep_assigned_version), f"Dependency {dep_package_name} version {dep_assigned_version} does not satisfy range {dep_range}"
...
def test_linear_dependency_chain(num_packages, seed):
    ...
    solution_packages = set(a.package.name for a in result.solution.get_all_assignments())
