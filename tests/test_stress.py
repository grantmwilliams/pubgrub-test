...
def create_wide_dependency_tree(width: int, depth: int) -> SimpleDependencyProvider:
    ...
    for level in range(1, depth + 1):
        ...
        for i in range(width):
            ...
            prev_level_pkg = packages_by_level[level - 1][i % len(packages_by_level[level - 1])]
            dep_range = VersionRange(ver, ver, True, True)
...
def create_complex_version_scenario(num_packages: int, versions_per_package: int) -> SimpleDependencyProvider:
    ...
    for i in range(1, num_packages):
        ...
        if versions_per_package > 1:
            ...
            broad_range = VersionRange(
                Version("1.0.0"), 
                Version(f"{versions_per_package}.0.0"), 
                True, True
            )
...
class TestStressTests:
    ...
    def test_linear_chain_performance(self):
        ...
        for length in chain_lengths:
            ...
            assert result.success, f"Resolution failed for chain length {length}: {result.error}"
            assert duration < 1.0, f"Resolution took too long ({duration:.3f}s) for chain length {length}"
            
            # Should have all packages in solution
            if result.solution:
                solution_size = len(result.solution.get_all_assignments())
                assert solution_size == length, f"Expected {length} packages, got {solution_size}"
    ...
    def test_wide_dependency_tree_performance(self):
        ...
        for width, depth in scenarios:
            ...
            assert result.success, f"Resolution failed for {width}x{depth} tree: {result.error}"
            assert duration < 2.0, f"Resolution took too long ({duration:.3f}s) for {width}x{depth} tree"
            
            # Should have reasonable number of packages in solution
            if result.solution:
                solution_size = len(result.solution.get_all_assignments())
                expected_min = depth + 1  # At least root + one from each level
                assert solution_size >= expected_min, f"Expected at least {expected_min} packages, got {solution_size}"
    ...
    def test_many_versions_scenario(self):
        ...
        for num_packages, versions_per_package in scenarios:
            provider, root_pkg, root_ver = create_complex_version_scenario(num_packages, versions_per_package)
            ...
            assert isinstance(result.success, bool), f"Invalid result for {num_packages}x{versions_per_package} scenario"
            assert duration < 3.0, f"Resolution took too long ({duration:.3f}s) for {num_packages}x{versions_per_package} scenario"
            
            if result.success and result.solution:
                solution_size = len(result.solution.get_all_assignments())
                assert solution_size <= num_packages, f"Too many packages in solution: {solution_size} > {num_packages}"
    ...
    def test_repeated_resolution_consistency(self):
        ...
        for i, result in enumerate(results[1:], 1):
            assert result.success == first_result.success, f"Result {i} success differs from first"
            ...
            if result.success and first_result.success:
                ...
                assert first_solution == result_solution, f"Result {i} solution differs from first"
    ...
    def test_very_large_scenario(self):
        ...
        assert result.success, f"Resolution failed for very large scenario: {result.error}"
        assert duration < 10.0, f"Resolution took too long ({duration:.3f}s) for very large scenario"
