...
def compare_results(python_result, rust_result):
    ...
    comparison = {
        "scenario": python_result.get(
            "scenario", rust_result.get("scenario", "unknown")
        ),
        "description": python_result.get(
        ...
        "agreement": {
            ...
            "solution_match": python_result.get("solution")
            == rust_result.get("solution"),
            "both_succeeded": python_result.get("success")
            and rust_result.get("success"),
            "both_failed": not python_result.get("success")
            and not rust_result.get("success"),
        },
        "performance": {
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python compare_implementations.py <scenario_file> [scenario_file2] ..."
        )
        sys.exit(1)
    ...
    for scenario_file in scenario_files:
        ...
        print(
            f"  {status} Agreement: {comparison['agreement']['success_match']}, "
            f"Python: {python_result.get('runtime_ms', 0):.1f}ms, "
            f"Rust: {rust_result.get('runtime_ms', 0):.1f}ms",
            file=sys.stderr,
        )
