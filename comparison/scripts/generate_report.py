...
"""
    ...
    both_succeeded = sum(
        1 for r in comparison_results if r["agreement"]["both_succeeded"]
    )
    both_failed = sum(1 for r in comparison_results if r["agreement"]["both_failed"])

    python_total_time = sum(
        r["performance"]["python_runtime_ms"] for r in comparison_results
    )
    rust_total_time = sum(
        r["performance"]["rust_runtime_ms"] for r in comparison_results
    )

    avg_speedup = sum(
        r["performance"]["speedup_factor"]
        for r in comparison_results
        if r["performance"]["speedup_factor"] > 0
    ) / max(
        1, sum(1 for r in comparison_results if r["performance"]["speedup_factor"] > 0)
    )
...
"""
    ...
    for result in comparison_results:
        ...
        if agreement["success_match"]:
            ...
            status_text = (
                "✓ Agreement" if agreement["both_succeeded"] else "✓ Both Failed"
            )
        else:
...
def main():
    if len(sys.argv) < 2:
        print(
            "Usage: python generate_report.py <comparison_results.json> [output_prefix]"
        )
        sys.exit(1)
